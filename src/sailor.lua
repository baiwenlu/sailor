...
--        title = conf.sailor.app_name,
    ...
    res = remy.splitstring(page.r.uri,"/")
    module = res[2]
    conf = require( module..".conf.conf")

    page.theme = conf.sailor.theme
    page.layout = conf.sailor.layout
    page.title = conf.sailor.app_name
    ngx.print(page.title)
    ngx.log(ngx.WARN,"SSSAAAAAAAAAAAAA "..page.title)
...
--[[
--**  @ 路由方法，通过page变量中保存的path信息，找到相应的controller
--]]
...
--    apache_friendly_url(page)
    --baiwenlu  20150907
    ngx.print("AAAAAAA  ",conf.sailor.friendly_urls)
    local route_name = ""

    if conf.sailor.friendly_urls  then
        route_name = page.r.uri
        res = remy.splitstring(route_name, "/")
        cjson = require "cjson"
        ngx.print("SSSSSSSSS ",cjson.encode(res))
    else
        route_name = page.GET[conf.sailor.route_parameter]
    end
